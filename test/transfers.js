...
contract('Transfers', (accounts) => {
  ...
  it('Initial transfers should fail but succeed after white listing', async () => {
    ...
    await shouldFail.reverting.withMessage(tokenInstance.transfer(accounts[2], 100, { from: accounts[0] }), FAILURE_NON_WHITELIST_MESSAGE)

    // Approve a transfer from account0 and then try to spend it
    await tokenInstance.approve(accounts[2], 100, { from: accounts[0] })
    await shouldFail.reverting.withMessage(tokenInstance.transferFrom(accounts[0], accounts[2], 100, { from: accounts[2] }), FAILURE_NON_WHITELIST_MESSAGE)

    // Add address 0 to whitelist
    await tokenInstance.addToWhitelist(accounts[0], 20, { from: accounts[1] })
    ...
    await tokenInstance.transferFrom(accounts[0], accounts[2], 100, { from: accounts[2] })
    ...
    await shouldFail.reverting.withMessage(tokenInstance.transfer(accounts[0], 100, { from: accounts[2] }), FAILURE_NON_WHITELIST_MESSAGE)

    // Should fail with transfer from going back to account 0 from 2
    await tokenInstance.approve(accounts[0], 100, { from: accounts[2] })
    await shouldFail.reverting.withMessage(tokenInstance.transferFrom(accounts[2], accounts[0], 100, { from: accounts[0] }), FAILURE_NON_WHITELIST_MESSAGE)
  })
})
